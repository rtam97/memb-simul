    ...
    if options['-o']:
        ...
        for mi in range(len(molecules)):
            ...
            if mol not in moleculeTypes or options['SeparateTop']:
                ...
                topdir = "./protein-topologies/%s-top/"%(options['-o'].value[:-4])
                if not os.path.isdir(topdir):
                    from os import mkdir
                    mkdir(topdir,777)
                destination = options["-o"] and open(topdir+moleculeTypes[mol]+".itp", 'w') or sys.stdout
        ...
        # Output stream
        top  = options["-o"] and open("./protein-topologies/%s"%options['-o'].value, 'w') or sys.stdout

        # ITP file listing
        itps = '\n'.join(['#include "'+topdir+'%s.itp"' % molecule for molecule in set(moleculeTypes.values())])
