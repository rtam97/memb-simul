...
if top:
    ...
    topmolecules = [i for i in top.molecules]
    topmoltypes = [i for i in top.moleculetypes]
    if options["-atomlist"]:
...
for residue,bb,nterm,cterm in zip(struc.residues,struc.backbone,struc.nterm,struc.cterm):
    ...
    print "Residue #%i of %i"%(counter,sizze)
    ...
    o, r = mapping[resn].do(counter,residue,target,bb,nterm,cterm,options["-nt"])
    out.extend(o)
    raw.extend(r)
